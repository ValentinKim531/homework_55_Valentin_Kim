...
class TodoForm(forms.ModelForm):
    ...
    text = forms.CharField(max_length=3000, required=False, label="Текстовое поле",
                           widget=widgets.Textarea)
    status = forms.ChoiceField(label="Статус", choices=StatusChoice.choices)
    execution_date = forms.CharField(max_length=10, required=False, label="Дата исполнения")
